...
const T = {
  pt: {
    ...
    'ind.desc.bollinger_band_width': 'Quão distantes estão a banda superior e a banda inferior da Bollinger Bands, em % da banda inferior — moedas em squeeze (bandas coladas) vs. moedas em expansão de volatilidade (bandas bem abertas). A coluna Larg% mostra a média dos últimos 5 candles fechados (reativa, sem ficar presa a um único candle isolado); N (lookback) é usado só pra calcular a faixa mín/máx da janela, não entra na média. Mostra a distância na tabela e permite ordenar por mais distantes ou mais próximas. Útil pro trade que entra na banda inferior e sai na superior. Padrão: 5min, BB(20,2), 100 candles — 1min fica disponível pra selecionar manualmente, mas escaneia todo o mercado mais devagar.',
  ...
  en: {
    ...
    'ind.desc.bollinger_band_width': 'How far apart the upper and lower Bollinger Bands are from each other, in % of the lower band — coins in a squeeze (tight bands) vs. coins in a volatility expansion (wide-open bands). The Larg% column shows the average of the last 5 closed candles (reactive, not stuck on a single candle); N (lookback) only sets the min/max range window, it does not feed the average. Shown in the table, sortable by farthest or closest. Useful for trades that enter at the lower band and exit at the upper band. Default: 5min, BB(20,2), 100 candles — 1min stays available to pick manually, but scans the whole market more slowly.',
