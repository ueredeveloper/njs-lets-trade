 ...
 */
export function buildBollingerBandWidthFilterName(interval, period, stdDev, lookback, opts = {}) {
 ...
 */
export function buildVwapBandExpansionFilterName(interval, vwapInterval, lookback, multiplier) {
  return `${interval}|vwapexp|${vwapInterval}|${lookback}|${multiplier}`;
}

export function parseVwapBandExpansionFilterName(name) {
  const parts = String(name).split('|');
