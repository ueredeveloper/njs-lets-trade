import {
  getBbFavSortOption, cycleBbFavSort, BB_FAV_SORT_OPTIONS,
} from '../utils/bollingerFavoritesSort';
...
export default function BollingerFavSortSelect({ value, onChange, className = '' }) {
  const { t } = useI18n();
  const opt = getBbFavSortOption(value);
  const idx = BB_FAV_SORT_OPTIONS.findIndex(o => o.id === value);
  const pos = `${(idx < 0 ? 0 : idx) + 1}/${BB_FAV_SORT_OPTIONS.length}`;

  function step(direction) {
    onChange(cycleBbFavSort(value, direction));
  }
  ...
  return (
    ...
    >
      <button
        type="button"
        className={ARROW_BTN}
        aria-label={t('bbfav.sort.prev')}
        title={t('bbfav.sort.prev')}
      ...
      <button
        type="button"
        className={ARROW_BTN}
        aria-label={t('bbfav.sort.next')}
        title={t('bbfav.sort.next')}
        onClick={(e) => { e.stopPropagation(); step(1); }}
      >
        <Chevron dir="right" />
      </button>
