...
import { useBollingerBandWidthMeta } from '../hooks/useBollingerBandWidthMeta';
import {
...
export default function CurrencyTable({ activeFilter, onSelectFilter, onSelectCurrency }) {
  ...
  const bbFavConfigs = useMemo(() => {
    ...
    for (const e of multitradeFavorites) {
      if (e.enabled === false || e.strategyId !== 'bollinger-bands') continue;
      const interval = e.entry?.interval ?? '4h';
  ...
  const {
    meta: bbFavWidthMeta,
    loading: bbFavWidthLoading,
  } = useBollingerBandWidthMeta(isBollingerBandsFavView, bbFavConfigs);
  ...
  const activeGrowthFilter = useMemo(() => {
    if (!activeFilter || favoriteView) return null;
    const f = findFilter(activeFilter);
    if (!f || !parseIndicatorGrowthFilterName(f.name)) return null;
    return f;
  }, [activeFilter, favoriteView, findFilter]);
  ...
  const activeBbWidthFilter = useMemo(() => {
    if (!activeFilter || favoriteView) return null;
    const f = findFilter(activeFilter);
    if (!f || !parseBollingerBandWidthFilterName(f.name)) return null;
    return f;
  }, [activeFilter, favoriteView, findFilter]);
  ...
  const rows = useMemo(() => {
    ...
    } else if (favoriteView === 'vwap-bands') {
      const vbEntries = multitradeFavorites.filter(e => e.enabled !== false && e.strategyId === 'vwap-bands');
      const vbSymbols = vbEntries.map(e => e.symbol);
      const vbPhaseBySymbol = new Map(vbEntries.map(e => [e.symbol, e.phase ?? 'WATCHING']));
      list = resolveFavorites(new Set(vbSymbols), currencies.list, gateAll);
      const have = new Set(list.map(c => c.symbol));
    ...
    } else if (activeBbWidthFilter && bbWidthMeta && sortVolume === 'none' && !favoriteView) {
      list = list.slice().sort((a, b) => {
        const wa = bbWidthMeta[a.symbol]?.avgWidthPct ?? (bbWidthSort === 'far' ? -Infinity : Infinity);
  ...
  // Pré-carrega Gate.io — necessário para favoritos G, MC, TX, AT e filtros Gate
  useEffect(() => {
  ...
  useEffect(() => {
    const needGate = favoriteView === 'gate' || favoriteView === 'macross'
      || favoriteView === 'vwap-bands'
      || favoriteView === 'trades' || favoriteView === 'active'
  ...
  // Carrega e refresca saldos reais a cada 30s enquanto o filtro AT estiver ativo
  useEffect(() => {
    if (favoriteView !== 'active') return undefined;
    refreshActiveTrades();
    const id = setInterval(refreshActiveTrades, 30_000);
    return () => clearInterval(id);
  }, [favoriteView, refreshActiveTrades]);

  // Contagem AT no toolbar: um carregamento inicial (sem poll contínuo)
  useEffect(() => {
    refreshActiveTrades();
  }, [refreshActiveTrades]);
  ...
  async function handleSelect(item, source = null) {
    ...
    try {
      ...
      if (isTradesFavView) {
        ...
        // com ciclo ativo (PENDING/BOUGHT), com ordem limite armada (WATCHING + entryLimit —
        // caso do bollinger-bands-bot.js, que não tem fase PENDING) ou que falhou (FAILED —
        // mostra o momento do sinal mesmo quando a entrada não deu certo) — WATCHING puro não
        // tem sinal/compra pra marcar.
        ...
        // no período das pernas (a 1ª busca acima é "últimos N até agora" e não alcança), sempre
        // no intervalo do TRADE (effectiveInterval) — mesma lógica de loadMultitradeSymbolChart.
    ...
    <div id="currency-table" className="currency-table flex flex-col h-full min-h-0 overflow-hidden">
      ...
      <div id="currency-table-toolbar" className="currency-table-toolbar flex flex-col gap-1 px-2 py-1 border-b border-p2 shrink-0">
        ...
        <div className="flex gap-1.5 overflow-x-auto pb-0.5 -mx-1 px-1 scrollbar-thin touch-pan-x">
          {uiPrefs.visibleFavoriteButtons.active !== false && (
          <ToolbarBtn
            id="currency-table-btn-filter-active"
            active={favoriteView === 'active'}
            color={ACTIVE_COLOR}
            label="AT"
            count={activeCount}
            title={favoriteView === 'active' ? t('fav.toolbar.active_on') : t('fav.toolbar.active', activeCount)}
            onClick={() => handleToggleFavoriteView('active')}
          />
          )}
          {uiPrefs.visibleFavoriteButtons.trades !== false && (
          <ToolbarBtn
          ...
          {uiPrefs.visibleFavoriteButtons['vwap-bands'] !== false && (
          <ToolbarBtn
            id="currency-table-btn-filter-vwap-bands"
            active={favoriteView === 'vwap-bands'}
            color={VWAP_BANDS_COLOR}
            label="VWAP"
            count={vwapCount}
          ...
          {uiPrefs.visibleFavoriteButtons.binance !== false && (
          <ToolbarBtn
          ...
          <thead className="sticky top-0 z-30 bg-p1">
            <tr className="lt-table-head">
              ...
              {isBbWidthFilter && (
                <th
                  className="text-right px-2 py-1 text-p5 opacity-80 font-normal uppercase tracking-wider whitespace-nowrap cursor-pointer hover:opacity-100 select-none"
                  title="Ordenar por largura das Bandas de Bollinger: mais distantes ↔ mais próximas"
              ...
              {showVwapFavWidthCol && (
                <th
                  className="text-right px-2 py-1 text-p5 opacity-80 font-normal uppercase tracking-wider whitespace-nowrap"
                  title="Largura média das bandas de VWAP nos favoritos"
                >
                  Larg% {vwapFavSort === 'width_far' ? '↓' : '↑'}
                </th>
              )}
              {showBbFavWidthCol && (
                isBbFavPercentBSort ? (
                ...
                <tr
                  ...
                  className={`lt-table-row cursor-pointer transition-colors ${
                    activeRow === item.symbol
                      ? 'bg-p2/80 text-white'
                      : isActiveFavView || isActiveHolding
                ...
                >
                  ...
                  >
                    <div className="flex flex-nowrap items-center gap-0.5">
                      ...
                      {uiPrefs.visibleFavoriteButtons['vwap-bands'] !== false && (
                      <FavButton kind="VWAP" text="VW" symbol={item.symbol} active={isVwap} color={VWAP_BANDS_COLOR} label="VWAP Bands" onClick={() => {
                        ...
                        <span className="flex items-center gap-1 text-[9px] font-normal" style={{ color: ACTIVE_COLOR }}>
                          <span>
                            {activeInfo.exchange === 'gate' ? t('fav.active.exchange_gate')
                              : activeInfo.exchange === 'binance' ? t('fav.active.exchange_bnb')
                              : 'Gate+Bnb'}
                  ...
                  {isBbWidthFilter && (() => {
                    const widthPct = bbWidthMeta?.[item.symbol]?.avgWidthPct;
                    return (
                      <td
                        className="px-2 py-1 text-right font-mono text-[10px] font-semibold"
                        style={{ color: widthPct == null ? 'rgba(255,255,255,0.35)' : '#e2c341' }}
      ...
      {vwapModal && (
        <ModalPortal>
          <VwapBandsFavoriteModal
            symbol={vwapModal.symbol}
