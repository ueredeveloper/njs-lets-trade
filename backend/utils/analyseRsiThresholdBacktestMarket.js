...
async function analyseRsiThresholdBacktestMarket(options = {}) {
    ...
    return {
        ...
        supportResistance: srOpts?.enabled ? {
            ...
            stopTrailingEnabled: !!srOpts.stopEnabled && !!srOpts.stopTrailingEnabled,
            stopTrailingCoinStepPct: Math.max(0.1, Number(srOpts.stopTrailingCoinStepPct ?? 1)),
            stopTrailingStopStepPct: Math.max(0.1, Number(srOpts.stopTrailingStopStepPct ?? 1)),
        } : null,
