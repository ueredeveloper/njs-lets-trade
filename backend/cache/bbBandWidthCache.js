 ...
 * 100 candles) e a combinação padrão do filtro de busca de indicadores (5min, mesmo BB(20,2),
 * 100 candles) — trade BB entra na banda inferior e sai na superior, então 5min é o default
 * do filtro. O preset de 1min existe mas nasce desligado (ver DEFAULT_OFF em cacheSettings.js):
 * escanear os ~500 pares USDT nesse intervalo não cabe no orçamento da fila global de candles
 * (~24 req/min) sem ficar constantemente atrasado.
 */
/**
 * ttlMs é independente do intervalo do candle: manter a MÉTRICA (largura média de 100
 * candles) "fresca" a cada fechamento de candle de 1min/5min exigiria re-varrer os ~500
...
const CACHED_PRESETS = [
  ...
  { key: '5m|20|2|100', interval: '5m', period: 20, stdDev: 2, lookback: 100, settingId: 'bbBandWidth5m', ttlMs: 30 * 60_000 },
];
