...
/**
 * Hourly RSI Bot — múltiplas moedas com capital reservado por símbolo
 *
 * Filtro tendência : EMA200(1h) — só compra se preço > EMA200(1h)
 * Entrada          : RSI(14) 1h < 30
 * Saída            : RSI(14) 1h > 70  (sem stop-loss)
 * Capital          : reservado e acumulado por símbolo — atualizado no Supabase por id
 * Estado           : tabela hourly_bot_state (ver hourly-bot.sql)
 *
 * Uso:
 *   node backend/bot/hourlyRsiBot.js
 *
 * Para adicionar moedas: INSERT em hourly_bot_state no Supabase.
 * Para trocar exchange: editar coluna exchange ('binance' ou 'gate').
 */
...
const RSI_BUY            = 30;
const RSI_SELL           = 70;
const RSI_FAST_THRESHOLD = 68;     // acima disto → poll a cada 1 min para não perder RSI>70
const CANDLE_LIMIT       = 250;    // cobre EMA200 + warmup — serve para RSI e EMA juntos
const POLL_MS            = 5 * 60_000; // 5 min — poll normal
const FAST_POLL_MS       = 60_000;     // 1 min — poll quando RSI >= 68
...
function makeLogger(symbol, color = '') {
  const logFile = path.join(BOT_DIR, `log-${symbol}-hourly.txt`);
...
async function loadSymbols() {
  return sbReq('GET', 'hourly_bot_state', null, '?order=id.asc');
}

// Atualiza por id — garante que cada moeda edita apenas sua própria linha
async function saveState(id, update) {
  await sbReq('PATCH', 'hourly_bot_state', { ...update, updated_at: new Date().toISOString() }, `?id=eq.${id}`);
}

async function saveTrade(trade) {
  await sbReq('POST', 'hourly_bot_trades', trade);
}

// ── Tick ──────────────────────────────────────────────────────────────────────
// Retorna o RSI calculado (ou prevRsi se não foi possível calcular)
// para que startSymbol ajuste o intervalo de polling dinamicamente.
async function tick(rowId, adapter, log, prevRsi = null) {
  // 1h candles cobrem RSI(14) e EMA200 em uma única busca
  const candles = await adapter.fetchCandles(CANDLE_LIMIT, INTERVAL);
  const closes  = candles.map(c => c.close);

  if (closes.length < EMA_PERIOD + RSI_PERIOD) { log('Dados insuficientes — aguardando.'); return prevRsi; }
  ...
  if (rsi == null || ema200 == null) { log('Indicadores insuficientes.'); return prevRsi; }

  // Loga transição de modo: RSI cruzou o threshold 68
  if (prevRsi !== null) {
    if (rsi >= RSI_FAST_THRESHOLD && prevRsi < RSI_FAST_THRESHOLD)
      log(`⚡ RSI=${rsi.toFixed(2)} ≥ ${RSI_FAST_THRESHOLD} — acelerando verificação para 1 min`);
    else if (rsi < RSI_FAST_THRESHOLD && prevRsi >= RSI_FAST_THRESHOLD)
      log(`🔄 RSI=${rsi.toFixed(2)} < ${RSI_FAST_THRESHOLD} — voltando a ${POLL_MS / 60_000} min`);
  }

  // Recarrega estado fresco do Supabase a cada tick
  const rows = await sbReq('GET', 'hourly_bot_state', null, `?id=eq.${rowId}&limit=1`);
  const state = rows?.[0];
  ...
  log(
    ...
    `  ${trendColor}${bullish ? '↑ ALTA' : '↓ BAIXA'}${X}` +
  ...
  if (phase === 'WATCHING') {
    if (rsi >= RSI_BUY) return rsi;

    if (!bullish) {
      log(`${Y}⚠️  RSI(1h)=${rsi.toFixed(2)} < ${RSI_BUY} mas preço < EMA200 — tendência BAIXA, entrada bloqueada${X}`);
      sendWhatsApp(`⚠️ ${symbol} RSI<${RSI_BUY} mas tendência BAIXA (close=${close.toFixed(6)} < EMA200=${ema200.toFixed(6)}) — entrada bloqueada`);
      return rsi;
    }

    log(`${G}📍 RSI(1h)=${rsi.toFixed(2)} < ${RSI_BUY} + EMA200 confirmando ALTA — comprando ${parseFloat(capital).toFixed(4)} USDT${X}`);
    ...
    await saveState(rowId, {
      ...
      rsi_entry: rsi, ema200_entry: ema200,
    });
    ...
    log(`   EMA200(1h)  : ${ema200.toFixed(6)}`);
    log(`${'─'.repeat(60)}`);
  ...
  // ── BOUGHT: aguarda RSI > 70 ──────────────────────────────────────────────
    ...
    if (rsi <= RSI_SELL) {
      if (!bullish) log(`${Y}⚠️  Em posição. Tendência virou BAIXA — aguardando RSI>${RSI_SELL} para sair${X}`);
      return rsi;
    }

    log(`${R}📈 RSI(1h)=${rsi.toFixed(2)} > ${RSI_SELL} — vendendo ${state.buy_qty} ${symbol}${X}`);
    ...
    await saveTrade({
      ...
      ema200: parseFloat(state.ema200_entry ?? ema200), trend_bullish: bullish,
    });

    await saveState(rowId, {
      capital: capitalAfter, phase: 'WATCHING',
      buy_price: null, buy_qty: null, buy_usdt: null, buy_time: null,
      rsi_entry: null, ema200_entry: null,
    });
...
async function startSymbol(row, color) {
  ...
  log(`=== Iniciado | ${adapter.name} | par: ${adapter.pair} | RSI(${RSI_PERIOD},${INTERVAL}) | EMA${EMA_PERIOD}(${INTERVAL}) | poll normal: ${POLL_MS/60_000}min | poll rápido: 1min (RSI≥${RSI_FAST_THRESHOLD}) | fase: ${row.phase} ===`);
  if (row.phase === 'BOUGHT') {
    log(`♻️  Posição aberta — comprado a ${parseFloat(row.buy_price).toFixed(6)} em ${row.buy_time} | qty=${row.buy_qty}`);
  }
  ...
  const schedule = () => {
    const fast  = lastRsi !== null && lastRsi >= RSI_FAST_THRESHOLD;
  ...
  const run = async () => {
    try {
      ...
      else if (errCount === 4) log(`❌ Erros repetidos — silenciando ${row.symbol}. Verifique se o par existe na exchange.`);
    }
    schedule();
  };

  await run(); // primeira execução imediata; schedule() agenda a próxima
}
...
async function main() {
  ...
  if (!rows?.length) {
    console.error('❌ Nenhum símbolo em hourly_bot_state. Execute hourly-bot.sql no Supabase.');
    process.exit(1);
  }

  console.log(`\n🤖 Hourly RSI Bot`);
  console.log(`   Entrada : RSI(${RSI_PERIOD}, ${INTERVAL}) < ${RSI_BUY}  +  preço > EMA${EMA_PERIOD}(${INTERVAL})`);
  console.log(`   Saída   : RSI(${RSI_PERIOD}, ${INTERVAL}) > ${RSI_SELL}  (sem stop-loss)`);
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    console.log(`   ${color}${row.symbol}${X}  exchange=${row.exchange ?? 'binance'}  capital=$${parseFloat(row.capital).toFixed(2)}  vol24h=${volFmt}  fase=${row.phase ?? 'WATCHING'}`);
