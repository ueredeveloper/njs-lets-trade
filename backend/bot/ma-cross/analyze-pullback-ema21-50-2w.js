'use strict';
/**
 * Mesmos trades reais do ma-cross (entrada 1h, como está hoje), testando a regra de pullback
 * com DOIS alvos — EMA21(1h) e EMA50(1h) — em vez de só um:
 *
 ...
 *   2) No candle do cruzamento, define o CANAL entre as duas EMAs:
 *        piso = min(EMA21, EMA50) × (1 - TOLERANCE_PCT%)
 *        teto = max(EMA21, EMA50) × (1 + TOLERANCE_PCT%)
 *      (TOLERANCE_PCT padrão 0.5% — folga pra fora dos dois lados do canal formado pelas
 *      próprias EMAs, em vez de uma banda fixa em torno de uma EMA só).
 *      - Se o close do candle do cruzamento já está dentro do canal (piso <= close <= teto)
 *        → entrada DIRETA, no close desse candle.
 *      - Se o close está FORA do canal (acima do teto ou abaixo do piso) → espera até
 *        WAIT_CANDLES candles (padrão 5) mirando o preço entrar na banda individual de
 *        QUALQUER uma das duas EMAs (±TOLERANCE_PCT em torno da EMA21 OU da EMA50 — mesma
 *        checagem "banda de dois lados" de antes, não mais o canal). Entra no close do
 *        primeiro candle que satisfizer. Se passar os 5 candles sem entrar na banda de
 *        nenhuma das duas, cancela (sinal descartado).
 ...
 * entrada (direta / espera-21 / espera-50 / cancelada por não voltar a tempo).
 *
 * Uso: node backend/bot/ma-cross/analyze-pullback-ema21-50-2w.js
 *      node backend/bot/ma-cross/analyze-pullback-ema21-50-2w.js --days 14 --tolerance-pct 0.5 --wait-candles 5
 */
...
const FAST_PERIOD = 9;
const SLOW_PERIOD = 21; // também é um dos dois alvos de pullback
const FILTER_PERIOD = 50; // segundo alvo de pullback
const FINE_IV = '5m';
const TP_PCT = 5;
...
function gapPct(close, ma) {
  return ma != null ? ((close - ma) / ma) * 100 : null;
}

/** TOLERANCE_PCT é banda de dois lados: até TOLERANCE_PCT abaixo OU acima da MA. */
function inBand(gap) {
  return gap != null && gap >= -TOLERANCE_PCT && gap <= TOLERANCE_PCT;
}

/**
 * Resolve a entrada hipotética a partir do cruzamento:
 *   - direta se o close já está dentro do canal formado pelas duas EMAs — piso = a menor das
 *     duas (EMA21/EMA50), teto = a maior das duas, cada uma com folga de TOLERANCE_PCT pra
 *     fora (0.5% abaixo do piso, 0.5% acima do teto);
 *   - senão (close fora do canal, pra cima ou pra baixo) espera até WAIT_CANDLES candles
 *     mirando o preço entrar na banda individual de QUALQUER uma das duas (±TOLERANCE_PCT
 *     em torno da EMA21 OU da EMA50, mesma checagem de antes); cancela se não voltar a tempo.
 */
function resolveEntry(candles, fastSeries, slowSeries, ema50Series, cross) {
  const ema21AtCross = maValueAt(slowSeries, cross.openTime);
  const ema50AtCross = maValueAt(ema50Series, cross.openTime);
  if (ema21AtCross == null || ema50AtCross == null) {
    return { bucket: 'sem_dados', reason: 'EMA21/EMA50 indisponível no candle do cruzamento' };
  }

  const g21 = gapPct(cross.close, ema21AtCross);
  const g50 = gapPct(cross.close, ema50AtCross);

  const channelFloor = Math.min(ema21AtCross, ema50AtCross) * (1 - TOLERANCE_PCT / 100);
  const channelCeiling = Math.max(ema21AtCross, ema50AtCross) * (1 + TOLERANCE_PCT / 100);
  const inChannel = cross.close >= channelFloor && cross.close <= channelCeiling;

  if (inChannel) {
    return {
      bucket: 'direta', gap21Pct: g21, gap50Pct: g50,
      target: ema21AtCross <= ema50AtCross ? 'ema21' : 'ema50',
  ...
  if (targetIdx >= candles.length) {
    return { bucket: 'sem_dados', gap21Pct: g21, gap50Pct: g50, reason: 'candles insuficientes após o cruzamento' };
  }

  for (let i = cross.index + 1; i <= targetIdx; i++) {
    const ema21i = maValueAt(slowSeries, candles[i].openTime);
    const ema50i = maValueAt(ema50Series, candles[i].openTime);
    if (ema21i == null || ema50i == null) continue;

    const gi21 = gapPct(candles[i].close, ema21i);
    const gi50 = gapPct(candles[i].close, ema50i);
    if (inBand(gi21) || inBand(gi50)) {
      return {
        bucket: 'espera', gap21Pct: g21, gap50Pct: g50,
        target: inBand(gi21) ? 'ema21' : 'ema50',
        gapAtEntryPct: inBand(gi21) ? gi21 : gi50,
  ...
  return {
    bucket: 'sem_retorno', gap21Pct: g21, gap50Pct: g50,
    reason: `preço não entrou na banda de ±${TOLERANCE_PCT}% da EMA21 nem da EMA50 em ${WAIT_CANDLES} candles`,
  };
}
...
async function analyzeTrade(trade) {
  ...
  const resolved = resolveEntry(candles, fastSeries, slowSeries, ema50Series, cross);
...
async function main() {
  ...
  console.log(`\n═══ ma-cross 1h — pullback com dois alvos (EMA21 e EMA50), tolerância ${TOLERANCE_PCT}%, espera ${WAIT_CANDLES} candles + saída stop 5% trailing / EMA9x21 cruza↓ / +5% ═══`);
  console.log(`Período dos trades reais: desde ${fmtDt(fromMs)} (${fromIso})`);
  console.log(`Regra: dentro do canal EMA21/EMA50 (±${TOLERANCE_PCT}% de folga) no cruzamento → direto | fora do canal → espera ${WAIT_CANDLES} candles mirando entrar na banda ±${TOLERANCE_PCT}% de qualquer uma das duas EMAs | não entrou → cancela\n`);
  ...
  const semRetorno = results.filter(r => r.bucket === 'sem_retorno');
  const semDados = results.filter(r => r.bucket === 'sem_dados');
  const entrariam = [...direta, ...espera];
  const naoEntrariam = [...semRetorno];

  console.log('\n── Resumo geral ──');
  console.log(`Trades reais no período: ${trades.length}`);
  console.log(`  Entrariam (direta + espera resolvida): ${entrariam.length}`);
  console.log(`    Direta (dentro do canal EMA21/EMA50, +-${TOLERANCE_PCT}% de folga): ${direta.length}`);
  console.log(`    Espera (entrou na banda de EMA21 ou EMA50 em ${WAIT_CANDLES} candles): ${espera.length}`);
  console.log(`  Não entrariam (preço não entrou na banda de nenhuma das duas em ${WAIT_CANDLES} candles): ${semRetorno.length}`);
  ...
  summarizeBucket(`Direta (canal EMA21/EMA50)`, direta);
  summarizeBucket(`Espera (voltou em ${WAIT_CANDLES} candles)`, espera);

  console.log('\n── Detalhe — entradas permitidas ──');
  console.log('Símbolo     | Cruzamento (1h)   | Bucket  | Alvo  | Gap21    | Gap50    | Entrada hipotética | Saída sim | PnL sim  | PnL real');
  console.log('------------|-------------------|---------|-------|----------|----------|---------------------|-----------|----------|----------');
  for (const r of entrariam) {
    console.log(
      `${r.symbol.padEnd(11)} | ${fmtDt(r.crossOpenTime).padEnd(17)} | ${r.bucket.padEnd(7)} | ${(r.target ?? '—').padEnd(5)} | `
      + `${fmtPct(r.gap21Pct).padStart(8)} | ${fmtPct(r.gap50Pct).padStart(8)} | ${fmtDt(r.hypEntryMs).padEnd(19)} | `
      + `${(r.exitType ?? '—').padEnd(9)} | ${fmtPct(r.simPnlPct).padStart(8)} | ${fmtPct(r.realPnlPct).padStart(8)}`,
    );
  }

  if (naoEntrariam.length) {
    console.log('\n── Detalhe — bloqueados (não entraram na banda de EMA21/EMA50 em tempo) ──');
    console.log('Símbolo     | Cruzamento (1h)   | Gap21    | Gap50    | PnL real (referência)');
    console.log('------------|-------------------|----------|----------|----------------------');
    for (const r of naoEntrariam) {
      console.log(`${r.symbol.padEnd(11)} | ${fmtDt(r.crossOpenTime).padEnd(17)} | ${fmtPct(r.gap21Pct).padStart(8)} | ${fmtPct(r.gap50Pct).padStart(8)} | ${fmtPct(r.realPnlPct).padStart(8)}`);
    }
  }
